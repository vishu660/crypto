...
class DistributeRoi extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($userPackages as $userPackage) {
            // ✅ Decode roi_dates safely
            $roiDatesRaw = $userPackage->roi_dates;

            $roiDates = is_string($roiDatesRaw)
                ? json_decode($roiDatesRaw, true)
                : $roiDatesRaw;

            if (!is_array($roiDates)) {
                Log::warning("Invalid ROI dates for UserPackage ID: {$userPackage->id}");
                continue;
            }
            ...
            if (in_array($today, $roiDates)) {
                ...
                // ✅ Create Wallet Entry with balance_after
                Wallet::create([
